...
class StreamingLlamaForCausalLM4K(LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.model = StreamingLlamaModel(config)
...
class StreamingLlamaModel(LlamaModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [StreamingLlamaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # these will be overwritten in StreamingLlamaDecoderLayer.forward
...
class StreamingLlamaDecoderLayer(LlamaDecoderLayer):
